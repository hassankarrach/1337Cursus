...
/*   Updated: 2024/01/27 05:25:50 by hkarrach         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	map_parsing(t_mlx *mlx, char c, int pos_x, int pos_y, int CoinFrame,
		int cat_frame, int exit_frame, int monster_frame)
{
	if (c == '1')
...
void	initialize_map(t_mlx *mlx, int CoinFrame, int cat_frame, int exit_frame,
		int monster_frame)
{
	int		pos_x;
	int		pos_y;
	...
	while (lines[pos_y])
	{
		...
		{
			map_parsing(mlx, lines[pos_y][pos_x], pos_x, pos_y, CoinFrame,
				cat_frame, exit_frame, monster_frame);
			pos_x++;
		}
		pos_y++;
	}
